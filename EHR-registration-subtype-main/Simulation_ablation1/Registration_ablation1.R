reg_simu <- function(seednum,generate_data,apply_time_shifts ) {
  ...
  # Function to calculate smooth-spline coefficients
  calculate_bspline_coefficients <- function(data,n_grid = 6,start_time = 1,end_time = 17) {
    uniq_times <- unique(data$Time)
    if (length(uniq_times) < 4) {
      last_time <- max(uniq_times)
      last_val  <- data$Value[which.max(data$Time)]
      eps <- 1
      n_missing <- 4 - length(uniq_times)
      new_times <- last_time + seq(eps, by = eps, length.out = n_missing)
      new_rows  <- data.frame(
        ID = data$ID[1],
        Feature = data$Feature[1],
        Time  = new_times,
        Value = rep(last_val, n_missing)
      )
      data <- rbind(data, new_rows)
    }
    grid_points <- seq(start_time, end_time, length.out = n_grid)
    fit <- smooth.spline(x  = data$Time, y  = data$Value, spar = 0.5)
    pred_vals <- predict(fit, x = grid_points)$y
    coeffs <- c(pred_vals)
    names(coeffs) <- c("(Intercept)", paste0("bsplines", seq_len(n_grid-1)))
    return(coeffs)
  }
