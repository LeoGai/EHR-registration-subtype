...
generate_data_S6 <- function(num_individuals, time_points, start_time, end_time, feature) {
  ...
  for (i in 1:num_individuals) {
    ...
    if (i <= 250) {
      ...
      individual_sequence <- 23 + base_sequence + rnorm(length(times), mean = 0, sd =0.8)
    } else if (i > 250 & i <= 500) {
      base_sequence <- 3*sin(0.6*(times+4))
      ...
      individual_sequence <- 25-0.5*times + base_sequence + rnorm(length(times), mean = 0, sd =0.8)
    }
...
apply_time_shifts_S6 <- function(data, num_individuals, timepos_shift, start_time, seed) {
  data_shifted <- data
  set.seed(seed)
...
clusterExport(cls, "generate_data_S6")
clusterExport(cls, "apply_time_shifts_S6")
Reg_Simu6 <- foreach(seednum = 1:500, .combine = 'rbind', .packages = c("cluster")) %dopar% reg_simu(seednum,generate_data=generate_data_S6,apply_time_shifts=apply_time_shifts_S6)
...
Result6 <- calculateStats(Reg_Simu6)

saveRDS(Reg_Simu6, file = paste0("./EHR-registration-subtype-main/Simulation_ablation1/FReg_Simu", 6, ".rds"))

saveRDS(Result6, file = paste0("./EHR-registration-subtype-main/Simulation_ablation1/FMatrix_Simu", 6, ".rds"))
