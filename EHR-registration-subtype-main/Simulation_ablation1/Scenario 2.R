...
generate_data_S2 <- function(num_individuals, time_points, start_time, end_time, feature) {
...
apply_time_shifts_S2 <- function(data, num_individuals, timepos_shift, start_time, seed) {
  data_shifted <- data
  set.seed(seed)
...
clusterExport(cls, "generate_data_S2")
clusterExport(cls, "apply_time_shifts_S2")
Reg_Simu2 <- foreach(seednum = 1:500, .combine = 'rbind', .packages = c("cluster")) %dopar% reg_simu(seednum,generate_data=generate_data_S2,apply_time_shifts=apply_time_shifts_S2)
stopCluster(cls)
...
Result2 <- calculateStats(Reg_Simu2)

saveRDS(Reg_Simu2, file = paste0("./EHR-registration-subtype-main/Simulation_ablation1/FReg_Simu", 2, ".rds"))

saveRDS(Result2, file = paste0("./EHR-registration-subtype-main/Simulation_ablation1/FMatrix_Simu", 2, ".rds"))
