...
generate_data_S3 <- function(num_individuals, time_points, start_time, end_time, feature) {
...
apply_time_shifts_S3 <- function(data, num_individuals, timepos_shift, start_time, seed) {
  data_shifted <- data
  set.seed(seed)
...
clusterExport(cls, "generate_data_S3")
clusterExport(cls, "apply_time_shifts_S3")

Reg_Simu3 <- foreach(seednum = 1:500, .combine = 'rbind', .packages = c("cluster")) %dopar% reg_simu(seednum,generate_data=generate_data_S3,apply_time_shifts=apply_time_shifts_S3)
stopCluster(cls)
...
Result3 <- calculateStats(Reg_Simu3)

saveRDS(Reg_Simu3, file = paste0("./EHR-registration-subtype-main/Simulation_ablation3/FReg_Simu", 3, ".rds"))

saveRDS(Result3, file = paste0("./EHR-registration-subtype-main/Simulation_ablation3/FMatrix_Simu", 3, ".rds"))
