...
reg_simu <- function(seednum) {
  ...
  data$GroupCategory <- cut(data$ID,
                            breaks = c(-Inf,300,550, 800, Inf),
  ...
  data_recover<-SubtypeAware_Registration(data=data_shifted,alpha=0.95,k_range=2:8,timepos_option=timepos_option,tau=0.45,tmin=1,tmax=17,nots=c(8,13))
...
saveRDS(Reg_k4, file = paste0("./EHR-registration-subtype-main/Comparison_DTW/Reg_k4",".rds"))
...
print(summary_stats_k4)

saveRDS(summary_stats_k4, file = paste0("./EHR-registration-subtype-main/Comparison_DTW/summary_stats_k4", ".rds"))