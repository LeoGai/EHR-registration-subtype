...
generate_data_SA13 <- function(num_individuals, time_points, start_time, end_time, feature) {
...
apply_time_shifts_SA13 <- function(data, num_individuals, timepos_shift, start_time, seed) {
  data_shifted <- data
  set.seed(seed)
  ...
  n_outliers <- ol
  outlier_indices <- sample(1:nrow(data_shifted), n_outliers)
  data_shifted$Value[outlier_indices] <- data_shifted$Value[outlier_indices] * 2
...
for (w in 1:3) {
  ...
  clusterExport(cls, "outl")
  clusterExport(cls, "w")
  clusterExport(cls, "ol")
  clusterExport(cls, "generate_data_SA13")
  clusterExport(cls, "apply_time_shifts_SA13")
  saresult <-  foreach(seednum = 1:500, .combine = 'rbind', .packages = c("cluster")) %dopar% reg_simu(seednum,generate_data=generate_data_SA13,apply_time_shifts=apply_time_shifts_SA13)
...
saveRDS(saresult, file = paste0("./EHR-registration-subtype-main/Sensitivity analyses/SA_1_3/Reg_SA", w, ".rds"))
assign(paste0("Matrix_SA", w), Result)
saveRDS(Result, file = paste0("./EHR-registration-subtype-main/Sensitivity analyses/SA_1_3/Matrix_SA", w, ".rds"))
