...
generate_data_SA1521 <- function(num_individuals, time_points, start_time, end_time, feature) {
...
apply_time_shifts_SA1521 <- function(data, num_individuals, timepos_shift, start_time, seed) {
  data_shifted <- data
  set.seed(seed)
...
clusterExport(cls, "pm")
clusterExport(cls, "w")
clusterExport(cls, "generate_data_SA1521")
clusterExport(cls, "apply_time_shifts_SA1521")

saresult <-  foreach(seednum = 1:500, .combine = 'rbind', .packages = c("cluster")) %dopar% reg_simu(seednum,generate_data=generate_data_SA1521,apply_time_shifts=apply_time_shifts_SA1521,M1=(pm[w-14]))
...
saveRDS(saresult, file = paste0("./EHR-registration-subtype-main/Sensitivity analyses/SA_15_21/Reg_SA", w, ".rds"))
assign(paste0("Matrix_SA", w), Result)
saveRDS(Result, file = paste0("./EHR-registration-subtype-main/Sensitivity analyses/SA_15_21/Matrix_SA", w, ".rds"))
