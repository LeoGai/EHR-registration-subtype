reg_simu <- function(seednum,generate_data,apply_time_shifts,alpha = 0.95,M1=8,tau=0.45 ) {
  ...
  repeat {
    ...
    if(M1 ==2){
      if (optimal_k == previous_k && all(sort(silhouette_widths, decreasing = TRUE)[1] <= sort(s_old, decreasing = TRUE)[1])) {
        break
      }
    }else{
