...
saveRDS(AKI_data_recover,"./EHR-registration-subtype-main/Real_data/AKI_DA_RE/AKI_data_recover.rds")

write.csv(AKI_data_recover, file = "./EHR-registration-subtype-main/Real_data/AKI_DA_RE/AKI_data_recover.csv", row.names = FALSE)
...
ICU_ID_SC_CATEGORY <- readRDS("./EHR-registration-subtype-main/Real_data/ICU_ID_SC_CATEGORY_1118.rds")

AKI_CAT<-ICU_ID_SC_CATEGORY
names(AKI_CAT) <- c("ID", "SC_CATEGORY")

AKI_data_raw_filtered_Clinic<-AKI_data_raw_filtered %>%
  inner_join(AKI_CAT, by = "ID")

AKI_data_raw_filtered_Clinic <- AKI_data_raw_filtered_Clinic  %>%
...
AKI_data_raw_filtered_Clinic_7day<-AKI_data_raw_filtered_Clinic%>%
  filter(Time <= tar_day)


bspline_coefficients_list_Clinic_7day<-map_bspline_7d(AKI_data_raw_filtered_Clinic_7day)

coefficients_df0_Clinic_7day <- data.frame(ID = integer(), Coef1 = numeric(), Coef2 = numeric(), Coef3 = numeric(), Coef4 = numeric(), Coef5 = numeric(), Coef6 = numeric())

for (i in 1:length(bspline_coefficients_list_Clinic_7day)) {
  original_coefficients <- bspline_coefficients_list_Clinic_7day[[i]]$Original
  coef1 <- original_coefficients["(Intercept)"]
  coef2 <- original_coefficients["bsplines1"]
  coef3 <- original_coefficients["bsplines2"]
  coef4 <- original_coefficients["bsplines3"]
  coef5 <- original_coefficients["bsplines4"]
  coef6 <- original_coefficients["bsplines5"]
  coefficients_df0_Clinic_7day <- rbind(coefficients_df0_Clinic_7day, data.frame(ID = i, Coef1 = coef1, Coef2 = coef2, Coef3 = coef3, Coef4 = coef4, Coef5 = coef5, Coef6 = coef6))
}

data_clustering_Clinic_7day <- coefficients_df0_Clinic_7day[, c("Coef1", "Coef2", "Coef3", "Coef4", "Coef5", "Coef6")]

data_clustering_Clinic_7day[,7]<-unique(AKI_data_raw_filtered_Clinic_7day$ID)  
colnames(data_clustering_Clinic_7day)[7]<-"ID"
...
for (g in 1:5) {
  ...
  data_clustering_Clinic_7day_sc<- data_clustering_Clinic_7day %>%
    inner_join(AKI_CAT, by = "ID")
  
  data_clustering_Clinic_7day_sc$SC_CATEGORY <- recode(data_clustering_Clinic_7day_sc$SC_CATEGORY,
  ...
  silhouette_res_Clinic <- silhouette(data_clustering_Clinic_7day_sc$SC_CATEGORY , dist(data_clustering_Clinic_7day_sc[,1:6]))
  cat("Raw Data Group",g,":")
  cat(as.numeric(round(mean(silhouette_res_Clinic[, "sil_width"]),3)), "\n")
...
AKI_recover_SC<-AKI_data_recover[,c(1:4,6)]
...
AKI_data_recover_Clinic<-AKI_data_recover %>%
