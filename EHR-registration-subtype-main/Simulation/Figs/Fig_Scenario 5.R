...
library(patchwork)
  ...
  p1<-ggplot(data_new_four1, aes(x = Time, y = Value, group = ID, color = GroupCategory)) +
    geom_line(size = 0.2) +
    geom_point(size = 0.5) +
    labs(title = "Scenario 5 True Trajectories", x = "Time", y = "Value", color = "Group") +
  ...
  p2<-ggplot(data_new_four_shifted, aes(x = Time, y = Value, group = ID)) +
    ...
    labs(title = "Scenario 5 Observed Trajectories", x = "Time", y = "Value")
  ...
  p3<-ggplot(data_recover, aes(x = Time, y = Value, group = ID, color = GroupCategory)) +
    geom_line(size = 0.2) +
    geom_point(size = 0.5) +
    labs(title = "Scenario 5 Registered Trajectories", x = "Time", y = "Value", color = "Group")  +
...
library(patchwork)
combined_plot_S5 <- p2 |p3 | p1   
ggsave(filename = "./EHR-registration-subtype-main/Simulation/Figs/NEWFIGS/NEWFig_S5.png",
       plot = combined_plot_S5,
       width = 13,  
       height = 4,  
       dpi = 300    
)
