...
library(patchwork)
  ...
  for (i in 1:num_individuals) {
    ...
    individual_data <- data.frame(ID = rep(i, length(times)), Feature = rep(feature, length(times)), Time = times, Value = individual_sequence)
  ...
  p1<-ggplot(data1, aes(x = Time, y = Value, group = ID, color = as.factor(ID <= 500))) +
    geom_line(size = 0.2) +
    geom_point(size = 0.5) +
    labs(title = "Scenario 1 True Trajectories", x = "Time", y = "Value", color = "Group") +
  ...
  p2<-ggplot(data_shifted, aes(x = Time, y = Value, group = ID)) +
    ...
    labs(title = "Scenario 1 Observed Trajectories", x = "Time", y = "Value")
  ...
  p3 <- ggplot(data_recover, aes(x = Time, y = Value, group = ID, color = as.factor(ID <= 500))) +
    geom_line(size = 0.2) +
    geom_point(size = 0.5) +
    labs(title = "Scenario 1 Registered Trajectories", x = "Time", y = "Value", color = "Group") +
...
combined_plot_S1 <- p2 |p3 | p1

ggsave(filename = "./EHR-registration-subtype-main/Simulation/Figs/NEWFIGS/NEWFig_S1.png",
