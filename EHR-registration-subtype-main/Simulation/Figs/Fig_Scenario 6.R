...
library(patchwork)
  ...
  p1<-ggplot(data_new_four1, aes(x = Time, y = Value, group = ID, color = GroupCategory)) +
    geom_line(size = 0.2) +
    geom_point(size = 0.5) +
    labs(title = "Scenario 6 True Trajectories", x = "Time", y = "Value", color = "Group") +
  ...
  # Assign random time shifts to each individual
  set.seed(seed)
  ...
  p2<-ggplot(data_new_four_shifted, aes(x = Time, y = Value, group = ID)) +
    ...
    labs(title = "Scenario 6 Observed Trajectories", x = "Time", y = "Value")
  ...
  p3 <- ggplot(data_recover, aes(x = Time, y = Value, group = ID, color = GroupCategory)) +
    geom_line(size = 0.2) +
    geom_point(size = 0.5) +
    labs(title = "Scenario 6 Registered Trajectories", x = "Time", y = "Value", color = "Group")  +
    scale_color_manual(values = c("green", "orange", "purple", "royalblue")) +
    theme(
      legend.position = c(0.99, 1), 
      legend.justification = c(0.99, 1), 
      legend.key.size = unit(0.5, "lines"), 
...
library(patchwork)
combined_plot_S6 <- p2 |p3 | p1  

ggsave(filename = "/hpc/home/xg83/EHR-registration-subtype-main/Simulation/Figs/NEWFIGS/NEWFig_S6.png",
       plot = combined_plot_S6,
       width = 13,  
       height = 4,  
       dpi = 300    
)
