...
generate_data_S4 <- function(num_individuals, time_points, start_time, end_time, feature) {
...
apply_time_shifts_S4 <- function(data, num_individuals, timepos_shift, start_time, seed) {
  data_shifted <- data
  set.seed(seed)
...
clusterExport(cls, "generate_data_S4")
clusterExport(cls, "apply_time_shifts_S4")

Reg_Simu4 <- foreach(seednum = 1:500, .combine = 'rbind', .packages = c("cluster")) %dopar%  reg_simu(seednum,generate_data=generate_data_S4,apply_time_shifts=apply_time_shifts_S4)
...
Result4 <- calculateStats(Reg_Simu4)

saveRDS(Reg_Simu4, file = paste0("./EHR-registration-subtype-main/Simulation_ablation2/FReg_Simu", 4, ".rds"))

saveRDS(Result4, file = paste0("./EHR-registration-subtype-main/Simulation_ablation2/FMatrix_Simu", 4, ".rds"))
