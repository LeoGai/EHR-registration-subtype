reg_simu <- function(seednum,generate_data,apply_time_shifts ) {
  ...
  library(glmnet)
  library(kernlab) 
  # Function to calculate RKHS coefficients
  calculate_bspline_coefficients <- function(data) {
    ...
    t <- data$Time
    y <- data$Value
    kfun <- rbfdot(sigma = sigma)
    K   <- kernelMatrix(kfun, matrix(t, ncol = 1))
    n   <- length(t)
    alpha <- solve(K + lambda * diag(n), y)
    grid_points <- seq(start_time, end_time, length.out = n_grid)
    K_grid      <- kernelMatrix(kfun,
                                matrix(grid_points, ncol = 1),
                                matrix(t,           ncol = 1))
    preds <- as.vector(K_grid %*% alpha)
    coeffs <- c(preds)
